		...
		if (traceRay(worldMap, rayDir, camera.pos, &octreeHit, hitPos, normal) == true)
		{
			...
			//for (int i = 0; i < pointLights.size() && hitByLight == false; ++i)
			{
				...
				//if ((glm::dot(-normal, pointLights[i]) + mDist) < 0)
				{
					...
					rayDir = glm::normalize(hitPos - glm::dvec3(pointLights[0]));

					traceRay(worldMap, rayDir, pointLights[0], &oHitLight, filler, filler2);
					...
					//if (octreeHit == oHitLight)
					{
						...
						// diffuse 
						//normal = -normal;
						glm::dvec3 lightDir = glm::normalize(pointLights[0] - hitPos);
						...
						double spec = pow(std::max(dot(viewDir, reflectDir), 0.0), 32.0);
