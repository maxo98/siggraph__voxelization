...
VoxelScene::VoxelScene()
{
	...
	loadModel(glm::dvec3(0, 0, 5), "duck.txt");
...
void VoxelScene::addPoint(glm::dvec3 pos, glm::vec3 color, const unsigned int maxLevel)
{
	...
	if (currentTree->contains != OCTREE_CONTENT::FILLED)//Should be empty or filled if we are at max level 
	{
		...
		currentTree->object = new glm::vec3(color);
	}
	else {
...
bool VoxelScene::loadModel(glm::dvec3 pos, std::string file)
{
	...
	if (fs.is_open() == true)
	{
		...
		while (fs.eof() == false)
		{
			...
			addPoint(glm::dvec3(stod(s2)/4+0.01, stod(s3)/4+0.01, stod(s1)/4+0.01) + pos, glm::vec3(stof(s4), stof(s5), stof(s6)), stoi(s7));
		}
